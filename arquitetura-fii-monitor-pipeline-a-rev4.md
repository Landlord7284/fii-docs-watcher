...
Arquitetura e condições de contorno para implementação. **Revisão 4.**
...
## 0. O que mudou na revisão 4

A revisão 4 introduz **dois perfis de execução** sobre um mesmo acervo e formaliza a **única exceção**
ao papel puramente detectivo da listagem global. A revisão 3 permanece íntegra como documento
(`arquitetura-fii-monitor-pipeline-a-rev3.md`); esta revisão a substitui como fonte de verdade.

| Área | Mudança | Motivo |
|---|---|---|
| **Perfis de execução** | `run` (varredura diária, com auditoria) e `run --monitor` (perfil frequente, sem auditoria). A janela de descoberta passa a ser configurável por perfil (`[discovery].days` e `[discovery].monitor_days`), com `1 <= monitor_days <= days <= retention.days` | Frescor ao longo do dia sem multiplicar o custo da varredura completa |
| **Janela de consulta (4.2)** | O invariante "toda execução consulta a janela de retenção inteira" passa a valer para a **varredura**; o monitor consulta uma janela deliberadamente mais estreita | A janela estreita é cobertura trocada por frequência, paga pela regra do watermark abaixo |
| **Watermark (4.3)** | Só avança quando a janela varrida **alcança a fronteira de retenção** — derivado das duas janelas, nunca do perfil | Uma varredura estreita não observou nada sobre os dias que pulou; registrá-la manteria o único alarme de lacuna permanentemente verde |
| **Descoberta do monitor (4.7, nova)** | O monitor lê a listagem global **em ordem decrescente de `dataEntrega`**, para na primeira linha já contabilizada por um **cursor** por tipo de fundo, e casa as linhas novas com os nomes monitorados para decidir **quais** consultas por entidade gastar | Custo por disparo independente do tamanho da lista monitorada — a consulta por entidade custa o mesmo em qualquer janela, então estreitar a janela não economizava nada |
| **Exceção à 4.5** | A listagem global passa a poder **selecionar** consultas por entidade no perfil frequente. Segue **proibida de rotear** documento para o acervo | Exceção estreita e delimitada: um nome não casado custa latência, nunca completude — a varredura diária continua consultando toda entidade incondicionalmente |
| **Varredura obrigatória** | Com o monitor habilitado, a varredura diária **não pode** ser desabilitada; o agendador recusa a combinação | A varredura é quem absorve toda falha do casamento de nomes; sem ela, os erros do portão deixariam de ser latência e virariam perda silenciosa |
| **Grafia da fonte** | A descoberta por entidade atualiza `descricao_fundo_fnet` a partir da linha mais recente observada (reforça 4.6) | O portão do monitor casa contra a grafia armazenada; uma renomeação nunca regravada cegaria o portão para sempre |

### Histórico — o que mudou na revisão 3
...
**Invariante (emendado na revisão 4):** a **varredura diária** consulta o intervalo `[hoje - (N-1), hoje]` por entidade, onde N é `[discovery].days` e segue `[retention].days` salvo configuração explícita. Não há intervalo incremental. O **perfil de monitoramento** (4.7) consulta uma janela deliberadamente mais estreita, `[discovery].monitor_days`, com `1 <= monitor_days <= days <= retention.days` — recusado na carga, nunca ajustado silenciosamente. Purga, índice `_inbox` e fronteira continuam medidos pela **janela de retenção**, e as duas janelas terminam no mesmo `hoje`.
...
**Invariante (novo na revisão 4):** uma varredura cuja janela **não alcança a fronteira de retenção** não avança o watermark, ainda que completa. A condição é derivada das duas janelas (`janela.primeiro_dia <= retenção.primeiro_dia`), nunca do perfil que executou: uma varredura estreita não observou nada sobre os dias que pulou, e registrar seu último dia afirmaria cobertura sobre dias que ninguém consultou — deixando o único alarme de lacuna do acervo permanentemente verde. Consequência deliberada: desabilitar a varredura diária faz o alerta de watermark disparar em toda execução.
...
**Invariante (emendado na revisão 4):** a auditoria é **detectiva, não corretiva**. Ela gera alerta para revalidação do escopo; nunca roteia documento por igualdade de texto para dentro do acervo, nem serve de caminho de descoberta. Falha da auditoria não é falha do job.

**Exceção única, formalizada nesta revisão:** o perfil de monitoramento (4.7) lê a listagem global para **selecionar quais consultas por entidade gastar** em cada disparo. A listagem **seleciona** consultas; **nunca roteia** documento — todo documento que entra no acervo continua vindo de uma consulta com `idFundo`, cuja resposta é conhecida a partir da pergunta. Um nome que o portão não casa custa **latência, nunca completude**: a varredura diária continua consultando toda entidade incondicionalmente e permanece a garantia de completude do acervo. Fora da 4.7, nada mais pode consumir a listagem global senão como auditoria.
...
Como o roteamento não depende mais do nome (4.1), uma renomeação não detectada deixou de causar perda silenciosa de documentos. O nome desatualizado afeta exibição, nomes de arquivo futuros, a auditoria da 4.5 e o portão da 4.7 — nos dois últimos, o efeito é um alerta ausente ou um documento atrasado até a próxima varredura, nunca perda.

**Invariante (novo na revisão 4):** a descoberta por entidade atualiza `descricao_fundo_fnet` a partir da linha mais recente que observou, persistindo a mudança no `funds.yaml`. É o que torna verdadeira a premissa "a varredura atualiza a grafia" de que o portão da 4.7 depende.

### 4.7 O perfil de monitoramento e a leitura da listagem (novo na revisão 4)

O robô tem **dois perfis** sobre um mesmo acervo. `run` é a varredura diária das seções 4.1–4.6, com auditoria. `run --monitor` é o perfil frequente: janela `[discovery].monitor_days`, sem auditoria, e com a descoberta abaixo. O flag carrega um **perfil, nunca um número** — a linha de cron diz qual perfil varre, e reajustar uma janela permanece uma edição de configuração.

A motivação é de custo: a consulta por entidade cobre a janela inteira em uma requisição, então **estreitar a janela não a barateia** — um monitor no formato da 4.1 custaria `entidades × disparos`, o mesmo trabalho mais vezes. A leitura da listagem faz o custo por disparo ser proporcional à **taxa de publicação da fonte**, independente do tamanho da lista monitorada.

**Mecanismo, por disparo e por tipo de fundo monitorado:**

1. **Leitura do topo.** Uma requisição à listagem global (sem `idFundo`), ordenada por `o[0][dataEntrega]=desc`, `l=200`, sobre a janela do monitor.
2. **Parada por repetição.** Um **cursor** por tipo de fundo — o instante de `dataEntrega` mais recente já contabilizado, persistido no manifesto — encerra a paginação na primeira linha estritamente anterior a ele. Linhas empatadas no minuto do cursor são mantidas e ultrapassadas: um grupo de empate nunca é dividido, e a deduplicação é por `(id, versao)`, jamais pelo timestamp. No caso comum a parada acontece na primeira página.
3. **Portão por nome.** Cada linha nova tem `descricaoFundo` normalizado e comparado por **igualdade exata após normalização** — nunca substring — contra os nomes monitorados **do mesmo tipo de fundo**: a grafia da própria fonte (`descricao_fundo_fnet`) e a denominação registrada. Colisão de nomes aciona **todas** as entidades que casarem.
4. **Roteamento inalterado.** Para cada entidade casada roda a consulta normal da 4.1, com `idFundo`, sobre a janela do monitor — e **só isso** entra no manifesto. Nenhuma linha da leitura global vira documento.

**Invariante — integridade da leitura decrescente.** Uma leitura interrompida cedo não pode verificar cobertura por identidades contra `recordsFiltered` — a verificação que capturou a perda silenciosa da 9.5 — então a **validação de ordem é o substituto**, aplicada a toda leitura: `dataEntrega` não crescente dentro e entre páginas, nenhum `(id, versao)` repetido entre páginas, e `recordsFiltered` estável ao longo de uma leitura. Qualquer violação aborta a leitura com o cursor **congelado**. Violação de ordem ou identidade repetida significa contrato da fonte quebrado (`CRITICAL`, execução termina com código 1); variação de `recordsFiltered` é comportamento legítimo — uma reapresentação encolhe o total no meio da leitura — e aborta como aviso. Não há laço de retry: uma leitura abortada custa latência, e o próximo disparo relê a partir do cursor congelado.

**Invariante — avanço do cursor.** O cursor avança somente quando **ambas** as condições valem: a leitura global terminou íntegra (fronteira cruzada sob ordem validada, ou fim natural da listagem) **e** toda consulta por entidade que ela acionou concluiu sem erro. Linhas individualmente malformadas são registradas como estado parcial e contam como contabilizadas — como na varredura — para que uma linha defeituosa não envenene o cursor para sempre. O cursor nunca regride, e perdê-lo é inócuo: o fallback é uma leitura completa da janela do monitor.

**Invariante — sem fallback.** Uma leitura global que falhou não aciona nada e **não** recai na descoberta por entidade: isso custaria uma requisição por entidade exatamente no momento em que a fonte está doente, e manteria vivo o mecanismo que esta seção substitui. A varredura diária é o plano B, por construção.

**Invariante — a varredura é obrigatória.** Com o perfil de monitoramento habilitado, a varredura diária não pode ser desabilitada; um agendador deve **recusar a combinação** na partida (configuração inválida). Todo modo de falha do portão — renomeação, classe nova, grafia nunca aprendida, `descricaoFundo` vazio — é absorvido pela varredura consultando cada entidade incondicionalmente; sem ela, esses erros deixariam de ser latência e virariam perda, enquanto o acervo continuaria parecendo atual porque o monitor segue arquivando de hora em hora. Varredura sem monitor permanece válida: é o robô como sempre foi.

A ordenação decrescente foi medida limpa em 2026-08-27 (janela de 333 linhas, zero perda, conjunto idêntico ao da ordem crescente), mas é uma dependência empírica nova sobre uma fonte sem contrato: deve permanecer coberta por teste `live` e remedida em vez de confiada indefinidamente. Se a fonte a retirar, a mesma leitura pode ser feita sobre a ordem crescente verificada, por aritmética de offset a partir de `recordsFiltered`, ao custo de uma requisição extra.
